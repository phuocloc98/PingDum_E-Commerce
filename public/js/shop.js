fetch('../public/json/products.json')
    ...
    .then((data) => {
        ...
        /* RENDER GRID PAGE */
        renderProduct_grid(data.products, start_grid, end_grid);
        renderPagination(data.products, perPage_grid, insert_pangination_grid);
        changPage_grid(data.products);

        /* RENDER LIST PAGE */
        renderProduct_list(data.products, start_list, end_list);
        renderPagination(data.products, perPage_list, insert_pangination_list);
...
const close_overview = document.querySelector('.close-overview');
...
const insert_pangination_grid = document.querySelector('.grid-pagination ul');
const insert_pangination_list = document.querySelector('.list-pagination ul');
...
window.addEventListener('load', () => {
    let loading = setTimeout(showOverview, 3500);
    function showOverview() {
        overview.classList.add('active');
        clearTimeout(loading);
    }
})
close_overview.addEventListener('click', () => {
...
function getCurrentPage(currentPage,perPage) {
    start = (currentPage - 1) * perPage;
    end = currentPage * perPage;
}

function renderPagination(numberPages, perPage, placeInsert) {
    let totalPages = Math.ceil(numberPages.length / perPage);
    let html = '';
    ...
    placeInsert.innerHTML = html;
}

function renderProduct_grid(dataProducts, start, end) {
    html = '';
...
function changPage_grid(dataProducts) {
    ...
    currentPages_grid.forEach((value, index) => {
        value.addEventListener('click', () => {
            ...
            getCurrentPage(currentPage_grid, perPage_grid);
            renderProduct_grid(dataProducts, start, end);
...
function renderProduct_list(dataProducts, start,end) {
    html = '';
    const content = dataProducts.map((item, index) => {
        if (index >= start && index < end) {
            return `
...
function changPage_list(dataProducts) {
    ...
    currentPages_list.forEach((value, index) => {
        value.addEventListener('click', () => {
            ...
            getCurrentPage(currentPage_list, perPage_list);
            renderProduct_list(dataProducts, start, end);
...
function switchOption() {
    const checkActive = document.querySelector('.grid-list-option a');
    if (changeView.matches && checkActive.className === "active") {
...
function switchImg() {
    if (changeImg.matches) {
        document.querySelector('.banner-img img').src = "../public/img/breadcrumb-img.jpg";
    }
    else {
        document.querySelector('.banner-img img').src = "../public/img/sidebar-banner.gif";
    }
}
switchImg();
...
const speak = (text) => {
    ...
    utter.onend = () => {
        console.log('SpeechSynthesisUtterance.on');
    }
    utter.onerror = (err) => {
...
const handleVoice = (text) => {
    ...
    if(handledText.includes("look for")){
        const result = handledText.split('for')[1].trim();
        document.querySelector('#searchBar').value = result;
        const changEvent = new Event('keyup');
        document.querySelector('#searchBar').dispatchEvent(changEvent);

        toast({
            title: "Success!",
            message: `you're looking for ${result}`,
            type: "success",
            duration: 5000
        });
        speak(`you're looking for ${result}`);
        return;
    }

    if(handledText.includes("reset")){
        ...
        toast({
            title: "Success!",
            message: `complete reset find`,
            type: "success",
            duration: 5000
        });
        speak(`complete reset find`);
        return;
    }
    
    if(handledText.includes("buy")){
        const result = handledText.split('buy')[1].trim();
        document.querySelectorAll('.grid-single-img h2').forEach(value => {
            if(value.textContent.toLocaleLowerCase() == result){
                const changEvent = new Event('click');
                value.parentElement.nextElementSibling.lastElementChild.dispatchEvent(changEvent);
                speak(`complete buy ${result} into cart`);
            }
        })
        return;
    }
    toast({
        title: "Warning!",
        message: `The command: "${handledText.toUpperCase()}" went wrong`,
        type: "warning",
        duration: 5000
    });
...
document.querySelector('.fa-question').addEventListener('click', () =>{
    ...
    setTimeout(() => {
        document.querySelector('.notification-content').classList.add('active');
    }, 3000);
})
document.querySelector('.notification-box').addEventListener('click', () =>{
    document.querySelector('.notification').classList.remove('active');
    document.querySelector('.notification-content').classList.remove('active');
})
